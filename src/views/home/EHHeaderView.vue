    ...
    <div class="relative text-black shadow-md">
        ...
        <div id="topBar" class="flex items-center p-2 justify-between container mx-auto">
            ...
            <div id="menuToggle" class="relative block md:hidden">
                <button @click="toggleSidebar" class="hite text-2xl">
                ...
                <nav>
                    <div class="flex justify-center space-x-4">
                        ...
                        <div class="w-full max-w-sm min-w-[200px] relative ml-1">
                            <div class="flex items-center rounded shadow-sm overflow-hidden bg-white">
                                ...
                                <router-link to="/recherche">
                                    <button class="bg-customRed px-4 py-3 hite ml-2">
                                ...
                                <div v-for="itemY in menuYelp" :key="itemY" class="space-y-1">
                                    <h3 class="text-lg font-semibold text-gray-800 mt-2">
                                        <router-link :to="itemY.route"
                                            ...
                                            <BaseIcon :name="itemY.icon" customColor="text-gray-700" size="20"
                                                stroke-width="2" />
                                            <span
                                                class="text-xs text-gray-700 hover:bg-gray-300 ml-2 whitespace-nowrap">
                                                {{ itemY.label }}</span>
                                        </router-link>
                                    </h3>
                                </div>
                        ...
                        <router-link to="/comunity"
                            class=" hover:bg-white/20 px-4 py-2 rounded-sm font-medium font-poppins"
                            active-class="border-b-2 border-indigo-400">
                            {{ $t('navbar.writeReview') }}
                        </router-link>

                        <router-link to="/comunity"
                            class="hite hover:bg-white/20 px-4 py-2 rounded-sm font-medium font-poppins"
                            active-class="border-b-2 border-indigo-400">
            ...
            <div id="userManage" class="relative flex items-center space-x-4">
                <button>
                    <i class="fa fa-star  hover:text-gray-300 text-black"></i>
                </button>
                <Button variant="danger" class="font-poppins text-black">{{ $t('login') }}</Button>
                ...
                <div class="flex flex-col space-y-6 mt-10">
                    ...
                    <div class="relative group">
                        <button @click="isDropdownVisible = !isDropdownVisible"
                            :class="{ 'focus:outline-none bg-white/10 hite focus:ring-teal-500 transition': isDropdownVisible, 'hite': !isDropdownVisible }"
                            class="hover:bg-white/20 hover:focus px-4 py-2 rounded-sm font-medium font-poppins">
                            {{ $t('navbar.yelpProfessional') }}
                            <i class="fa fa-chevron-down text-xs ml-1 hite"></i>
                        </button>
                        ...
                        <div v-if="isDropdownVisible"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 gap-x-2 gap-y-2 p-3">
                                ...
                                <h3 class="text-lg font-semibold text-gray-800 mt-3">
                                    <router-link to=""
                                        class="flex justify-start item-center text-sm text-customRed hover:text-gray-900">
                                        <BaseIcon name="Telescope" customColor="text-gray-700" size="20"
                                            stroke-width="2" />
                                        <span
            ...
            <div class="w-full max-w-sm min-w-[200px] relative ml-4">
                <div class="flex items-center rounded shadow-sm overflow-hidden bg-white">
                    ...
                    <input v-model="rightValue" @click="activeInput = 'right'" placeholder="Yaoun"
                        class="w-1/2 px-3 py-2 text-sm text-gray-600 placeholder:text-gray-500 focus:outline-none" />
                    <router-link to="/recherche">
                        <button class="bg-customRed px-4 py-3 hite ">
                            <BaseIcon name="Search" size="20" stroke-width="2" />
                        </button>
                    </router-link>
                </div>
                ...
                <ul v-if="activeInput === 'left'" @click="activeInput = 'left'" @mouseleave="handleMouseLeave('left')"
                    ...
                    <li v-for="item in menuItems" :key="item"
                        @click="leftValue = item.label; selectItem('left', item.label)"
                        class=" flex justify-start items-center z-[100] px-3 py-2 text-sm hover:bg-gray-100 cursor-pointer ">
                ...
                <ul v-if="activeInput === 'right'" @click="activeInput = 'right'"
                    @mouseleave="handleMouseLeave('right')"
                    class="absolute right-0 w-1/2 bg-white border z-[100] rounded shadow-lg mt-1 overflow-auto  ">
                    <li class="flex justify-center items-center mt-3">
                        <BaseIcon name="MapPin" customColor="text-blue-500" size="20" stroke-width="3" />
                    ...
                    <li v-for="item in menuData" :key="item"
                        @click="rightValue = item.label; selectItem('right', item.label)"
