...
import HotelCardSkeleton from '@/components/skeletons/HotelCardSkeleton.vue'
...
const filteredHotels = computed(() => {
  ...
  const isSymbolRange = selectedRanges.every(
  ...
  if (selectedAmenities.length) {
    ...
    hotels = hotels.filter((hotel) => {
      ...
      return selectedAmenities.every((amenityId) => {
        ...
        console.log(
          `Checking hotel "${hotel.name}" for facility "${facilityName}": ${facilityFound}`
        );
...
onMounted(async () => {
  console.log(
    "Chargement des services avec les paramètres:",
  ...
  const guest_count = rooms.reduce(
    (sum, room) => sum + (room.adults || 0) + (room.childrens || 0),
    0
  );
  ...
  try {
    ...
    console.log("Services hotel:", sercivesHotels.value);
    console.log(
      "Services chargés (raw):",
      JSON.parse(JSON.stringify(services.value.data))
    );
    console.log("Services chargés utilisation:", services.value.data.hotels);
  } catch (error) {
    console.error("Erreur lors du chargement:", error);
  ...
  <div>
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-xl font-bold">
        ...
        <span v-if="sortedHotels.length"
          >{{ sortedHotels.length }} {{ $t("appServices.hotel.hotel")
          }}<span v-if="sortedHotels.length > 1">s</span>
          {{ $t("appServices.hotel.found")
          }}<span v-if="sortedHotels.length > 1">s </span></span
        >
        <span v-if="searchParams?.location && sortedHotels.length">
          {{ $t("to") }} {{ searchParams.location }}</span
        >
      </h2>
        ...
        <div class="relative">
          ...
          >
            <option value="recommended">
              {{ $t("appServices.hotel.recommended") }}
            </option>
            <option value="price-low">
              {{ $t("appServices.hotel.priceLowToHigh") }}
            </option>
            <option value="price-high">
              {{ $t("appServices.hotel.priceHighToLow") }}
            </option>
            <option value="rating">
              {{ $t("appServices.hotel.guestRating") }}
            </option>
          </select>
          <ArrowUpDownIcon
            size="16"
    ...
    <div>
      ...
      <div class="space-y-4">
        ...
        <template v-if="loading">
          <HotelCardSkeleton v-for="n in 3" :key="n" />
        </template>

        <!-- Hotels list -->
        <template v-else-if="sortedHotels.length > 0">
          <HotelCard
            v-for="hotel in paginatedServices"
          ...
          <div
            v-if="totalPages > 1"
            class="flex items-center justify-center space-x-2 mt-8 pb-5"
          >
            <button
