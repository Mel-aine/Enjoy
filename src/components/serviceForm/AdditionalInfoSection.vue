...
const paymentMethodsOptions = [
  { id: 'Credit Card', label: t('card'), icon: CreditCardIcon },
  { id: 'Cash', label: t('cash'), icon: DollarSignIcon },
  { id: 'Check', label: t('check'), icon: CheckIcon },
  { id: 'PayPal', label: t('paypal'), icon: AppleIcon },
  { id: 'smallpay', label: t('smallPay'), icon: SmartphoneIcon },
  { id: 'Orange Money', label: "Orange Money", icon: SmartphoneIcon },
  { id: 'Mobile Money', label: "Mobile Money", icon: SmartphoneIcon },
...
<template>
  <div class="space-y-6">
    ...
    <div class="bg-white p-4 rounded-lg border border-gray-200">
      <label for="policies" class="block text-sm font-medium text-gray-700">{{ $t('policiesTitle') }}<span
      ...
      <textarea id="policies" name="policies" rows="4"
        ...
        :placeholder="$t('policies.example')" required :value="formData.policies" @input="handleChange" />
      <p class="mt-1 text-sm text-gray-500">{{ $t('policiesPlaceholder') }}</p>
    </div>
    <div class="bg-white p-4 rounded-lg border border-gray-200">
      <fieldset>
        <legend class="text-sm font-medium text-gray-700">{{ $t('paymentTitle') }}<span class="text-red-500">*</span>
        </legend>
        <div class="mt-2 grid grid-cols-2 md:grid-cols-3 gap-y-2">
