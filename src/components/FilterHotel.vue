        ...
        <div class="space-y-2">
          <div v-for="rating in [5, 4, 3, 2, 1]" :key="rating" class="flex items-center">
            <input type="checkbox" :id="`star-${rating}`" v-model="starRating" :value="rating" 
              class="mr-2" @change="updateFilters" />
            <label :for="`star-${rating}`" class="flex items-center cursor-pointer">
              <StarIcon v-for="i in rating" :key="i" size="16" class="text-yellow-500 fill-yellow-500" />
      ...
      <div>
        ...
        <div class="space-y-2">
          <div v-for="amenity in amenitiesList" :key="amenity.id" class="flex items-center">
            <input type="checkbox" :id="amenity.id" v-model="amenities" :value="amenity.id" 
              class="mr-2" @change="updateFilters" />
