<template>
  <div
    class="flex flex-col md:flex-row items-center px-8 hover:shadow-lg cursor-pointer border-b  overflow-hidden md:w-[850px]  mx-auto w-[390px] transition-transform transform" @click="categoryFlags.isHotel?handleViewDeal():viewDetail()">
    <!-- Image / Carousel -->
    <div class="relative w-full md:w-1/2 lg:w-1/3">
      <!-- Carousel wrapper -->
      <div class="relative h-40 md:h-[200px] overflow-hidden rounded-lg">
        <div class="flex transition-transform duration-200 ease-linear"
          :style="{ transform: 'translateX(' + -currentIndex * 100 + '%)' }">
          <div v-for="(img, index) in service.images" :key="index" class="w-full flex-shrink-0">
            <img :src="img" alt="Menu Item" class="object-cover w-full h-48 md:h-40 lg:h-48 rounded-lg" />
          </div>
        </div>
      </div>

      <!-- Slider controls -->
      <button @click="prevSlide" :disabled="currentIndex === 0" type="button"
      ...
      <button @click="nextSlide" :disabled="currentIndex === props.service.images.length - 1" type="button"
    ...
    <div class="flex flex-col justify-between p-5 w-full h-[250px]">
      <router-link :to="search"
        ...
        <span class=" text-black hover:text-primary">{{ service.name }}</span>
      </router-link>

      <!-- Étoiles -->
      <div class="flex items-center mb-1 sm:mb-2 md:mb-1 gap-1">
        <span v-for="star in 5" :key="star" class="">
          <StarIcon colorLeft="rgba(25,31,52,0.6)" colorRight="rgba(25,31,52,0.6)" />
        </span>
      ...
      <div
        class="flex flex-col sm:flex-row sm:items-center gap-2 text-gray-600 dark:text-gray-400 text-xs md:text-sm mt-1">
        <div class="flex items-center text-purple-500">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-purple-500 mr-1" viewBox="0 0 24 24"
            fill="currentColor">
            <path
              d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5S10.62 6.5 12 6.5s2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z" />
          </svg>
          <span class="text-xs sm:text-sm truncate md:text-md">{{ service.address.text }}</span>
        </div>
        <div class="flex flex-col md:flex-row md:items-center md:justify-between w-full gap-2">
          <div class="flex items-center gap-2">
            <span class="text-customBlue font-bold text-xs sm:text-sm md:text-xs">{{ service.price }}</span>
          </div>
        </div>
      </div>

      <!-- Description -->
      <div class="mb-1 text-sm md:text-base font-normal text-gray-700 dark:text-gray-400 mt-1">
        <div class="items-start flex ">
          <div>
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-gray-500 mr-2 mt-1" viewBox="0 0 24 24"
              fill="currentColor">
        ...
        <!-- Bouton -->
        <div class="flex flex-row md:flex-nowrap lg:flex-nowrap gap-2 py-1 sm:py-5 md:py-2 lg:py-5">
          <RoundedButton v-for="item in category" :key="item.id" :label="$t('Category.' + item)" />
        </div>

      </div>
      <div class="flex justify-end" v-if="categoryFlags?.isHotel">
        <button @click="handleViewDeal" class="bg-primary text-white px-4 py-2 rounded-md hover:bg-primary/25 mt-2">
...
const categoryFlags = computed(() => {
  return getCategoryCheckers(props.service.categoryId);
})
...
const viewDetail = () => {
  hotelStore.setHotel(props.service)
  hotelStore.getHotelId(props.service.id)
  window.location.href = `/all_service/${props.service.id}`;
}
</script>
