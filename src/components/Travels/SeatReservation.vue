...
<template>
  <transition name="slide">
    <div v-if="isSidebarOpen" class="fixed inset-0 bg-black bg-opacity-40  flex justify-end">
      <!-- Sidebar -->
      <div
        class="w-[450px] h-full bg-gray-50 shadow-lg sm:p-4 p-3 rounded-l-lg overflow-auto scrollbar-hidden "
        @click.stop
      >
        <div class="p-4 min-h-screen">
          <div class="flex flex-row items-center mb-8">
            <div class="flex flex-col items-center mx-8">
              <span class="text-xl font-bold text-center">{{$t('reservation_siege')}}</span>
          ...
          <div class="justify-center border-2 p-4 self-center sm:w-full w-80 rounded-t-3xl border-gray-400">
          ...
          <div class="text-center mt-4 space-x-3 space-y-2">
            <p v-if="selectedSeats.length == 0"> {{ $t('aucun_siege') }}</p>
             <p v-else>{{ selectedSeats.length }} {{ $t('seat') }}{{ selectedSeats.length > 1? 's':'' }} {{$t('reserve')}}{{ selectedSeats.length>1? 's':'' }} </p>
            <!-- <button @click="$emit('cancel')" class="bg-gray-200 px-4 py-2 rounded">Annuler</button> -->
            <button @click="confirm" class="bg-green-500 text-white px-4 py-2 rounded">{{ $t('confirm') }}</button>
          </div>
        </div>
      </div>
    </div>
